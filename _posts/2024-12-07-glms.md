...
$$\begin{equation}\label{eq:normalerr} \varepsilon_i \sim \mathrm{Normal}(0, \; \sigma^2), \end{equation}$$
...
$$\begin{equation}\label{eq:normalerr2} y \sim \mathrm{Normal}\left(X\beta, \; \sigma^2 \mathbf{1}\right) \end{equation}$$
...
Using the fact that MLE is $\hat{\beta} = \left(X^T X\right)^{-1} X^T y$ together with a little bit of algebra exercise, 
we can obtain:

$$\begin{equation}\label{eq:beta_estimate} \hat{\beta} \sim \mathrm{Normal}\left(\beta, \; \sigma^2 \left(X^T X\right)^{-1}\right) \end{equation}$$
...
The theory around [standard error](https://en.wikipedia.org/wiki/Standard_error) elaborates on this.
...
$$\hat{\sigma}^2 = \frac{1}{n}\left(y-X\hat{\beta}\right)^T\left(y-X\hat{\beta}\right)$$

has the $\chi^2$ distribution variant:

$$\begin{equation}\label{eq:sigma_estimate} \hat{\sigma}^2 \sim \mathrm{Scaled–Inverse–}\chi^2(n-k, \; \sigma^2) \end{equation}$$

(_Side note:_ $\hat{\sigma}$ is biased, but it's more efficient than the unbiased sample variance formula with $n-1$ in denominator. This again illustrates that MLE prefers efficient estimates over unbiased.)
...
It is a bit more tedious calculation, but it can be done somewhat routinely to obtain that $\hat{y}$ has the [t-distribution](https://en.wikipedia.org/wiki/Student%27s_t-distribution) (shifted and scaled):

$$\begin{equation}\label{eq:y_predictive} \hat{y} \sim t_{n-k} \left(x^T\beta, \; \sigma^2 \frac{n}{n-k}\left(1 + x^T\left(X^T X\right)^{-1} x\right)\right) \end{equation}$$

where the parameters are expectation and variance. So when we want precise prediction bands, we can just use this. However, it may be a bit awkward to work with and it doesn't 
generalize well to other cases. 
So asymptotics to the rescue!
With growing number $n$ of samples in  $X \in \mathbb{R}^{n\times k}$, we are kinda back where we started and get
